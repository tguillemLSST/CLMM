...
def test_mock_data():
    ...
    def mass_mock_cluster(mass=15.,guess=15.):
        ...
        binned = cl.make_radial_profile("Mpc", bins=da.make_bins(0.5, 5.0, 40), 
    ...
    input_masses = np.array([13.82,15.49,13.93])
    guess_masses = np.array([15.70,13.90,14.07])
...
def test_z_distr():
    """
    Test the redshift distribution options: single plane, uniform, Chang13.
    """    
    ...
    data = mock.generate_galaxy_catalog(10**mass, 0.3, 4, cosmo, 'uniform', ngals=2600,
    ...
    assert_allclose(hist[0],100*np.ones(len(hist[0])),atol=200,rtol=0.02)
...
def test_shapenoise():
    ...
    data = mock.generate_galaxy_catalog(10**15., 0.3, 4, cosmo, 0.8, ngals=5000,shapenoise=0.5)
    # Check that there are no galaxies with |e|>1
    assert_equal(np.count_nonzero((data['e1']>1) | (data['e1']<-1)),0)
    assert_equal(np.count_nonzero((data['e2']>1) | (data['e2']<-1)),0)
    ...
    sigma=0.25
    data = mock.generate_galaxy_catalog(10**12., 0.3, 4, cosmo, 0.8, ngals=10000,shapenoise=sigma)
    # Check that there are no galaxies with |e|>1
    assert_equal(np.count_nonzero((data['e1']>1) | (data['e1']<-1)),0)
    assert_equal(np.count_nonzero((data['e2']>1) | (data['e2']<-1)),0)
    # Check that shape noise is Guassian with correct std dev
    bins=np.arange(-1,1.1,0.1)
    gauss = 1000*np.exp(-0.5*(bins[:-1]+0.05)**2/sigma**2)/(sigma*np.sqrt(2*np.pi))
