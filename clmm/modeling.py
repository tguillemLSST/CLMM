...
def predict_surface_density(r_proj, mdelta, cdelta, z_cl, cosmo, delta_mdef=200,
    ...
    if halo_profile_model.lower() == 'nfw':
        ...
        r3d = np.logspace(np.log10(np.min(r_proj))-1 , np.log10(np.max(r_proj))+1, len(r_proj)*10) 
        rho = get_3d_density(r3d, mdelta, cdelta, z_cl, cosmo, delta_mdef=delta_mdef, 
        ...
        integrand = rho/(omega_m_transformed*rhocrit_cosmo)
        sigma = ct.deltasigma.Sigma_at_R(r_proj, r3d, integrand, mdelta, cdelta, omega_m_transformed)
    else:
        raise ValueError(f"Profile model {halo_profile_model} not currently supported")
    return sigma
...
def predict_excess_surface_density(r_proj, mdelta, cdelta, z_cl, cosmo, delta_mdef=200,
    ...
    if halo_profile_model.lower() == 'nfw':
        ...
        rhocrit_mks = 3.*100.*100./(8.*np.pi*const.GNEWT.value)
        rhocrit_cosmo = rhocrit_mks * 1000. * 1000. * const.PC_TO_METER.value * 1.e6 / const.SOLAR_MASS.value
        r3d = np.logspace(np.log10(np.min(sigma_r_proj))-1 , np.log10(np.max(sigma_r_proj))+1, len(sigma_r_proj)*10)
        rho_einasto = get_3d_density(r3d, mdelta, cdelta, z_cl, cosmo, delta_mdef=delta_mdef,
                                     halo_profile_model=halo_profile_model, alpha=alpha)
        integrand = rho_einasto/(omega_m_transformed*rhocrit_cosmo)
